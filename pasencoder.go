...
// <--- paseto encoder --->
func EncodeWithRole(role, email, privatekey string) (string, error) {
	token := paseto.NewToken()
	token.SetIssuedAt(time.Now())
	token.SetNotBefore(time.Now())
	token.SetExpiration(time.Now().Add(2 * time.Hour))
	token.SetString("admin", email)
...
func DecodeGetAdmin(PublicKey, tokenStr string) (pay string, err error) {
...
func DecodeGetRoleandAdmin(PublicKey, tokenStr string) (pay string, use string) {
	key, err := Decoder(PublicKey, tokenStr)
	if err != nil {
		fmt.Println("Cannot decode the token", err.Error())
	}
	return key.Role, key.Admin
}
